...
router.get('/get', requireAdminMiddleware, async (request, response) => {
    try {
        ...
        if (!fs.existsSync(configPath)) {
            return response.status(404).json({ error: 'Configuration file not found' });
        }
        ...
        const oauthConfig = {
            ...
            linuxdo: {
                ...
                wellKnownEndpoint: config?.oauth?.linuxdo?.wellKnownEndpoint || '',
        ...
        return response.status(500).json({ error: 'Failed to load configuration' });
    }
});
...
router.post('/save', requireAdminMiddleware, async (request, response) => {
    try {
        ...
        if (!github || !discord || !linuxdo) {
            return response.status(400).json({ error: 'Missing required configuration fields' });
        }
        ...
        config.oauth.linuxdo = {
            ...
            wellKnownEndpoint: String(linuxdo.wellKnownEndpoint || ''),
        ...
        return response.json({
            success: true,
            message: 'OAuth configuration saved. Restart the service to apply changes.'
        });
    } catch (error) {
        console.error('Error saving OAuth config:', error);
        return response.status(500).json({ error: 'Failed to save configuration: ' + error.message });
    }
});
